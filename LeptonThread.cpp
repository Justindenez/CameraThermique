...
void LeptonThread::setAlpha(int newalpha)
{
	alpha = newalpha;
}
...
void LeptonThread::run()
{
	...
	uint16_t maxi = maxValue;
	uint16_t mini = minValue;
	...
	while(true) {
			...
			for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
				for(int i=0;i<FRAME_SIZE_UINT16;i++) {
					...
					if ((autoRangeMax == true) && (value > maxValue)) {
						maxValue = value;
						//maxi = lepton_temperature();
						maxi = maxValue;
						maxi = maxi/100 - 273.15;
						max = QString::number(maxi);
						emit updateTextMax(max);
					}
					if ((autoRangeMin == true) && (value < minValue)) {
						minValue = value;
						mini = minValue;
						mini = mini/100 - 273.15;
						min = QString::number(mini);
						emit updateTextMin(min);
					}
				}
			}
		...
		for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				//
				valueRaw = (valueFrameBuffer/100)-273;
				...
				color = qRgba(colormap[ofs_r], colormap[ofs_g], colormap[ofs_b],alpha);
				if (typeLepton == 3) {
				...
				//myImageRaw.setPixel(column,row, valueRaw);
				myImageRaw(column,row) = valueRaw;
				myImage.setPixel(column, row, color);
				//myImageRaw2.setPixel(column, row, valueRaw);
			}
		}
